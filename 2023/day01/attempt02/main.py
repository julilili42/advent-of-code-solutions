...
def part1(data):
    ...
    numeric_data = []
    # filters out all characters which are not numeric 
    for s in data:
        numeric_string = []
        for ch in s:
            if ch.isnumeric():
                numeric_string.append(ch)
        numeric_data.append("".join(numeric_string))
...
def part2(data):
    ...
    for s in data:
        ...
        for i in range(len(s)):
            if s[i].isnumeric():
                numeric_string.append(s[i])
            else:
                for num_string in map.keys():
                    if s.startswith(num_string, i):
                        numeric_string.append(map[num_string])
        numeric_data.append("".join(numeric_string))
